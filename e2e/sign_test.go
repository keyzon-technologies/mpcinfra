...
func testSigningOneNodeOffline(t *testing.T, suite *E2ETestSuite) {
	...
	if nodeToStop < len(suite.mpcinfraProcesses) && suite.mpcinfraProcesses[nodeToStop] != nil {
		err := suite.mpcinfraProcesses[nodeToStop].Process.Kill()
		if err != nil {
			t.Logf("Failed to stop node %d: %v", nodeToStop, err)
		} else {
			t.Logf("Successfully stopped node %d", nodeToStop)
			// Mark as stopped
			suite.mpcinfraProcesses[nodeToStop] = nil
		}
	}
