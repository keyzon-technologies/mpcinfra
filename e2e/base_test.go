...
func (s *E2ETestSuite) RegisterPeers(t *testing.T) {
	...
	// Use mpcinfra register-peers command instead of manual registration
	t.Log("Running mpcinfra-cli register-peers...")
...
func (s *E2ETestSuite) StartNodes(t *testing.T) {
	...
	s.mpcinfraProcesses = make([]*exec.Cmd, numNodes)
...
func (s *E2ETestSuite) StopNode(t *testing.T, nodeIndex int) {
	if nodeIndex >= len(s.mpcinfraProcesses) || s.mpcinfraProcesses[nodeIndex] == nil {
		t.Logf("Node %d is not running or doesn't exist", nodeIndex)
		return
	}

	cmd := s.mpcinfraProcesses[nodeIndex]
...
func (s *E2ETestSuite) KillAllMPCProcesses(t *testing.T) {
	...
	// Find all mpcinfra processes
	cmd := exec.Command("pgrep", "-f", "mpcinfra")
