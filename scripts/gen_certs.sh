...
echo ""
echo "── To use with docker-compose.prod.yml (base64 via .env) ──────────────────"
echo ""
echo "Run from the repo root:"
echo ""
echo "  # Server (NATS and Consul containers)"
echo "  echo \"NATS_TLS_CERT=\$(base64 -w 0 certs/nats-server.pem)\"     >> .env"
echo "  echo \"NATS_TLS_KEY=\$(base64 -w 0 certs/nats-server.key)\"      >> .env"
echo "  echo \"NATS_TLS_CA=\$(base64 -w 0 certs/rootCA.pem)\"            >> .env"
echo "  echo \"CONSUL_TLS_CERT=\$(base64 -w 0 certs/consul-server.pem)\"  >> .env"
echo "  echo \"CONSUL_TLS_KEY=\$(base64 -w 0 certs/consul-server.key)\"   >> .env"
echo "  echo \"CONSUL_TLS_CA=\$(base64 -w 0 certs/rootCA.pem)\"           >> .env"
echo ""
echo "  # Client (mpcinfra nodes connecting to NATS and Consul)"
echo "  echo \"NATS_CLIENT_CERT=\$(base64 -w 0 certs/nats-client.pem)\"      >> .env"
echo "  echo \"NATS_CLIENT_KEY=\$(base64 -w 0 certs/nats-client.key)\"       >> .env"
echo "  echo \"CONSUL_CLIENT_CERT=\$(base64 -w 0 certs/consul-client.pem)\"  >> .env"
echo "  echo \"CONSUL_CLIENT_KEY=\$(base64 -w 0 certs/consul-client.key)\"   >> .env"
echo "  echo \"TLS_CA=\$(base64 -w 0 certs/rootCA.pem)\"                     >> .env"
echo ""
echo "Then start:"
echo "  docker compose -f docker-compose.prod.yml up -d"
echo ""
echo "For Cloud you can: paste each value from the commands above into the service env vars dashboard."
