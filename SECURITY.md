# mpcinfra Security Model

## Core Security Principles

mpcinfra implements a threshold signature scheme with industry-standard security practices to protect cryptographic operations:
...
- **Identity Generation**: Secure identity creation with the `generate-identity` command:
  ```
  go run cmd/mpcinfra-cli/main.go generate-identity --node=node0 --peers=peers.json --encrypt
  ```
...
Every message in the mpcinfra network undergoes rigorous verification:
