...
# Configuration setup script for mpcinfra
# This script sets up mpcinfra configuration and systemd service (without starting)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
mpcinfra_HOME="/opt/mpcinfra"
SERVICE_NAME="mpcinfra"
...
# Prompt for mpcinfra_NODE_NAME if not provided
setup_node_name() {
    log_step "Setting up node name..."
    
    local node_name=""
    prompt_input "Enter mpcinfra_NODE_NAME:" "node_name" "Examples: node0, node1, node2, node3,.."
    
    # Store the node name for later use
    mpcinfra_NODE_NAME="$node_name"
    log_info "Node name set to: $mpcinfra_NODE_NAME"
}
...
check_binaries() {
    ...
    if [[ -f "/usr/local/bin/mpcinfra" ]] && [[ -f "/usr/local/bin/mpcinfra-cli" ]]; then
...
ensure_configuration() {
    ...
    mkdir -p "$mpcinfra_HOME"
    mkdir -p "/etc/mpcinfra"
    ...
    if [[ -f "/etc/mpcinfra/config.yaml" ]]; then
        log_info "Config file found at /etc/mpcinfra/config.yaml"
    else
        log_error "Config file not found at /etc/mpcinfra/config.yaml"
        log_error "Please create the config file before running this script:"
        if [[ "$mpcinfra_ENVIRONMENT" == "development" ]]; then
            log_error "  1. Copy the template: sudo cp config.yaml.template /etc/mpcinfra/config.yaml"
        else
            log_error "  1. Copy the template: sudo cp config.prod.yaml.template /etc/mpcinfra/config.yaml"
        fi
        log_error "  2. Edit the configuration: sudo vim /etc/mpcinfra/config.yaml"
        log_error "  3. Run this script again"
        exit 1
    fi
    
    # Set proper ownership and permissions for /etc/mpcinfra directory and config file
    # Note: This will be called after create_user() so mpcinfra group will exist
...
create_user() {
    ...
    if ! id "mpcinfra" &>/dev/null; then
        useradd -r -s /bin/false -d "$mpcinfra_HOME" -c "mpcinfra MPC Node" mpcinfra
        log_info "Created mpcinfra user and group"
    else
        log_info "User mpcinfra already exists"
    fi
    
    # Ensure group exists (in case user was created differently)
    if ! getent group mpcinfra >/dev/null; then
        groupadd -r mpcinfra
        usermod -g mpcinfra mpcinfra
        log_info "Created mpcinfra group and added user to it"
    fi
    
    # Set proper ownership for /opt/mpcinfra
    chown -R mpcinfra:mpcinfra "$mpcinfra_HOME"
    chmod 750 "$mpcinfra_HOME"
    
    # Set proper ownership and permissions for /etc/mpcinfra
    if [[ -d "/etc/mpcinfra" ]]; then
        chown root:mpcinfra "/etc/mpcinfra"
        chmod 750 "/etc/mpcinfra"
        log_info "/etc/mpcinfra directory permissions set (root:mpcinfra 750)"
        
        # Set permissions for config file if it exists
        if [[ -f "/etc/mpcinfra/config.yaml" ]]; then
            chown root:mpcinfra "/etc/mpcinfra/config.yaml"
            chmod 640 "/etc/mpcinfra/config.yaml"
            log_info "Config file permissions set (root:mpcinfra 640)"
        fi
    fi
}
...
check_encrypted_identity() {
    local current_node_name="$mpcinfra_NODE_NAME"
    local identity_dir="$mpcinfra_HOME/identity"
...
update_service_credentials() {
    ...
    local CRED_FILE="/etc/mpcinfra/mpcinfra-db-password.cred"
    local IDENTITY_CRED_FILE="/etc/mpcinfra/mpcinfra-identity-password.cred"
    local SERVICE_TEMPLATE="$SCRIPT_DIR/mpcinfra.service"
    local TARGET_SERVICE="/etc/systemd/system/mpcinfra.service"
    ...
    if check_encrypted_identity; then
        ...
        if [[ -f "$IDENTITY_CRED_FILE" ]]; then
            ...
            sed -i "s|--password-file=%d/mpcinfra-db-password.cred|--password-file=%d/mpcinfra-db-password.cred --identity-password-file=%d/mpcinfra-identity-password.cred --decrypt-private-key|g" "$TARGET_SERVICE"
            ...
            log_error "Please run setup-mpcinfra-cred.sh first to generate identity password credential"
            exit 1
        fi
    else
        ...
        sed -i "/SetCredentialEncrypted=mpcinfra-identity-password.cred:/d" "$TARGET_SERVICE"
        ...
        sed -i "s| --identity-password-file=%d/mpcinfra-identity-password.cred --decrypt-private-key||g" "$TARGET_SERVICE"
...
install_systemd_service() {
    ...
    if [[ ! -f "$SCRIPT_DIR/mpcinfra.service" ]]; then
        log_error "Service file not found: $SCRIPT_DIR/mpcinfra.service"
        exit 1
    fi
...
check_environment() {
    ...
    local config_file="/etc/mpcinfra/config.yaml"
...
validate_config_credentials() {
    ...
    local config_file="${1:-/etc/mpcinfra/config.yaml}"
    local errors=0
...
setup_environment_file() {
    ...
    local env_file="$mpcinfra_HOME/.env"
    ...
    cat > "$env_file" << EOF
# mpcinfra Environment Variables
# Generated on $(date)
# Note: All credentials are now configured in /etc/mpcinfra/config.yaml
mpcinfra_NODE_NAME=${mpcinfra_NODE_NAME}
EOF

    # Secure the environment file - only root can read/write, service user can read
    chown root:mpcinfra "$env_file"
    chmod 640 "$env_file"
...
verify_deployment() {
    ...
    for dir in "${required_dirs[@]}"; do
        if [[ ! -d "$mpcinfra_HOME/$dir" ]]; then
            log_error "Missing required directory: $mpcinfra_HOME/$dir"
            ((errors++))
        else
    ...
    # config.yaml is in /etc/mpcinfra/, others are in $mpcinfra_HOME
    if [[ ! -f "/etc/mpcinfra/config.yaml" ]]; then
        log_error "Missing required file: /etc/mpcinfra/config.yaml"
        ((errors++))
    else
    ...
    for file in "${required_files[@]}"; do
        if [[ ! -f "$mpcinfra_HOME/$file" ]]; then
            log_error "Missing required file: $mpcinfra_HOME/$file"
            ((errors++))
        else
    ...
    # Check binaries
    local required_binaries=("mpcinfra" "mpcinfra-cli")
    ...
    if [[ -f "$mpcinfra_HOME/peers.json" ]]; then
        # Parse peers.json to get node names
        local node_names
        if command -v jq >/dev/null; then
            node_names=$(jq -r 'keys[]' "$mpcinfra_HOME/peers.json" 2>/dev/null)
            if [[ $? -eq 0 ]] && [[ -n "$node_names" ]]; then
                ...
                while IFS= read -r node_name; do
                    ...
                    local identity_file="$mpcinfra_HOME/identity/${node_name}_identity.json"
                    ...
                    if [[ -f "$mpcinfra_HOME/.env" ]]; then
                        current_node_name_from_env=$(grep "^mpcinfra_NODE_NAME=" "$mpcinfra_HOME/.env" | cut -d'=' -f2 | tr -d '"'"'"' ')
                    fi
                    
                    if [[ "$node_name" == "$current_node_name_from_env" ]]; then
                        # This is the current node - private key is required
                        local private_key_plain="$mpcinfra_HOME/identity/${node_name}_private.key"
                        local private_key_encrypted="$mpcinfra_HOME/identity/${node_name}_private.key.age"
    ...
    if [[ -f "$mpcinfra_HOME/.env" ]]; then
        ...
        current_node_name=$(grep "^mpcinfra_NODE_NAME=" "$mpcinfra_HOME/.env" | cut -d'=' -f2 | tr -d '"'"'"' ')
        
        if [[ -n "$current_node_name" ]]; then
            log_info "Current node from .env: $current_node_name"
            
            local current_private_key_plain="$mpcinfra_HOME/identity/${current_node_name}_private.key"
            local current_private_key_encrypted="$mpcinfra_HOME/identity/${current_node_name}_private.key.age"
            ...
            log_warn "mpcinfra_NODE_NAME not found in .env file"
        fi
    fi
...
main() {
    log_info "Starting mpcinfra configuration setup..."
...
case "${1:-deploy}" in
    ...
    "validate-config")
        ...
        while true; do
            ...
            echo -e "    /etc/mpcinfra/config.yaml"
            echo -e "    ./config.yaml"
    ...
    "help"|"--help"|"-h")
        ...
        echo "  validate-only   Validate /etc/mpcinfra/config.yaml credentials only"
